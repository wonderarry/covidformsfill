...
def clear_snils(value):
    if value == 'nan':
        return 'skip'
    mutable_value = value.replace('-','').replace('_','').replace(' ','')
    if mutable_value.isdigit() and len(mutable_value) == 11:
        if mutable_value == '11111111111':
            return 'НЕКОРРЕКТНОЕ ЗНАЧЕНИЕ'
...
def row_to_dictionary(row):
    returned_dict = {
            ...
            #SNILS
            'snils_value':          clear_snils(row[row_indices['SNILS_ROW']]),
            #Commentary
